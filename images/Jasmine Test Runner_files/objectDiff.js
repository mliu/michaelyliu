...
objectDiff.diffOwnProperties = function diffOwnProperties(a, b, ignore) {
  ...
  keys.forEach(function (key) {
    var aValue, bValue;
      ...
      aValue = a[key];
      bValue = b[key];
    var typeA = typeof aValue;
    var typeB = typeof bValue;
		...
		if (b.hasOwnProperty(key)) {
			if (aValue === bValue) {
				...
				if (aValue && bValue && (typeA == 'object' || typeA == 'function') && (typeB == 'object' || typeB == 'function')) {
					var valueDiff = diffOwnProperties(aValue, bValue, ignore);
					if (valueDiff.changed == 'equal') {
						diff[key] = {
							changed: 'equal',
							value: aValue
		...
		for (var key in diff) {
			...
			switch (changed) {
				...
				case 'primitive change':
					properties.push(
						stringifyObjectKey(escapeHTML(key)) + '<span>: </span>' +
						'<del class="diff diff-key">' + inspect(diff[key].removed) + '</del><span> x </span>' +
						'<ins class="diff diff-key">' + inspect(diff[key].added) + '</ins>');
					break;
